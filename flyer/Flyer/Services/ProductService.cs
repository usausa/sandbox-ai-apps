...
public sealed class ProductService
{
    ...
    /// <summary>複数商品を一括登録（または更新）する。</summary>
    public async Task<int> RegisterProductsAsync(
        IEnumerable<(string Id, string Name, int Price, string? Category)> products,
        CancellationToken cancellationToken = default)
    {
        ...
        foreach (var (id, name, price, category) in products)
        {
            ...
            await RegisterProductAsync(id, name, price, category, cancellationToken).ConfigureAwait(false);
            count++;
        }
