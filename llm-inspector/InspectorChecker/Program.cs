...
using InspectorChecker.Components;
...
// Settings
var inspectorCheckerSettings = builder.Configuration.GetSection("InspectorChecker").Get<InspectorCheckerSettings>() ?? new InspectorCheckerSettings();
var foundrySettings = builder.Configuration.GetSection("Foundry").Get<FoundrySettings>() ?? new FoundrySettings();
builder.Services.AddSingleton(inspectorCheckerSettings);
...
builder.Services.AddSingleton<SurveyCsvLoader>();
builder.Services.AddSingleton<InspectionFeatureSummaryBuilder>();
builder.Services.AddSingleton<InspectionFraudAnalyzer>();
builder.Services.AddSingleton<InspectorCheckerService>();
...
var uploadDir = Path.GetFullPath(inspectorCheckerSettings.UploadPath);
